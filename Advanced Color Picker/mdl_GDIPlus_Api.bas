Attribute VB_Name = "mdl_GDIPlus_Api"
Option Explicit
...
Public Const AlphaShiftDiv = &H1000000
Public Const RedShiftDiv = &H10000
Public Const GreenShiftDiv = &H100&
Public Const BlueShiftDiv = &H1&
...
'' A faster version using shifts; only if you avoid Overflow
''  Hari Krishnan G. aka eXeption!
'Public Function ColorARGB(ByVal alpha As Byte, ByVal Red As Byte, ByVal Green As Byte, ByVal Blue As Byte) As Long
'   Dim result As Long
'   result = (alpha * AlphaShiftDiv) Or (Red * RedShiftDiv) Or (Green * GreenShiftDiv) Or (Blue * BlueShiftDiv)
'   ColorARGB = result
'End Function

Public Sub ARGBColor(ByVal color As Long, alpha As Byte, Red As Byte, Green As Byte, Blue As Byte)
   alpha = CByte((alpha And AlphaMask) \ AlphaShiftDiv)
   Red = CByte((Red And RedMask) \ RedShiftDiv)
   Green = CByte((Green And GreenMask) \ GreenShiftDiv)
   Blue = CByte((Blue And BlueMask) \ BlueShiftDiv)
End Sub
...
'   Hari Krishnan G.       [ aka eXeption ]
'**********************************************
' Modified functions from GetEncoderClsid()/GetDecoderClsid() in order to
'  return a string containing all the file extensions supported
Public Function GetEncoderList() As String
    ...
    ' Loop through all the codecs
    str = PtrToStrW(ICI(1).FilenameExtension)
    For i = 2 To num
        str = str & ";" & PtrToStrW(ICI(i).FilenameExtension)
    Next
...
Public Function toRGB(ByVal clr As Long, r, g, b)
    r = CLng(clr And &HFF&)
    g = CLng((clr And &HFF00&) \ &H100&)
    b = CLng((clr And &HFF0000) \ &H10000)
End Function
